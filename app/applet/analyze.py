...
for fn in funcs:
    ...
    if len(matches) <= 1:
        # Check if referenced as action string or in quotes or case-insensitive
...
# 3. Local variables inside functions
unused_locals = []
for idx, fn in enumerate(funcs):
    ...
    for rel_i, f_line in enumerate(func_lines, 1):
        ...
        for var_name in m_list:
            p = r'\b' + re.escape(var_name) + r'\b'
            m = re.findall(p, func_body)
            if len(m) <= 1:
                unused_locals.append({
                    'kind': 'variabel',
                    'name': var_name,
...
for ug in unused_globals:
    print(f"Name: {ug['name']} | Kind: {ug['kind']} | Line: {ug['line']} | Code: {ug['raw']}")

print("\n=== UNUSED LOCALS ===")
for ul in unused_locals:
    print(f"Name: {ul['name']} | Func: {ul['func']} | Line: {ul['line']} | Code: {ul['raw']}")
